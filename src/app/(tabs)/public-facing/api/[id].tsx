...
export async function loader(
  ...
  const response = await fetch(
    `${origin}/api/portfolio/${encodeURIComponent(routeId)}`,
    {
      cache: "no-store",
    },
  );
      ...
      {section.collapsible ? (
        ...
        >
          <View className="flex-row items-start justify-between gap-2">
            ...
            <ThemedText
              className="detail-body pt-0.5 text-lg md:text-xl"
              style={{ color: textColor }}
            >
              {isOpen ? "v" : ">"}
            </ThemedText>
        ...
        <ThemedText
          type="subtitle"
          className="detail-section-header mb-4 text-2xl md:text-3xl"
        >
          {section.title}
        </ThemedText>
      )}
...
function APIDetailContent() {
  ...
  const data =
    (useLoaderData<typeof loader>() as DetailData | undefined) ??
    fallbackDetail;
  ...
  const executeQuantumEndpoint = useCallback(
    async (input: {
      ...
      const { executeQuantumSdkEndpoint } =
        await import("@/lib/quantum-sdk-executor");
        ...
        <PortfolioSectionCard
          ...
          onToggle={() =>
            setExpandedSections((current) => ({
              ...current,
              [section.id]: !(
                current[section.id] ?? Boolean(section.defaultExpanded)
              ),
            }))
          }
        />
          ...
          <View className="gap-3">
            ...
            {registryEntry.portfolioUrl ? (
              <View>
                ...
                <ExternalLink
                  href={registryEntry.portfolioUrl}
